                ...
                foreach ($query->result() as $res) {
                    ...
                    if ($query2 = $this->pago_model->getPagoOT(" AND p.id_orden_trabajo=$row->id_orden_trabajo")) {
                        foreach ($query2->result() as $res2) {
                            ...
                            // $row->primer_abono = $res->FECHA_ORDEN_TRABAJO;
                            $row->primer_abono = $res2->FECHA_PRIMER_ABONO;
                        }
                    }

                    if ($query3 = $this->orden_trabajo_model->getDetalleEstados(" AND dot.id_orden_trabajo=$row->id_orden_trabajo")) {
                        foreach ($query3->result() as $res3) {
                            $row->entregado = $res3->ENTREGADO;
                            $row->pendiente = $res3->PENDIENTE;
                        }
                    }
                    ...
                    $row->deuda = $row->total-$row->monto_total;
