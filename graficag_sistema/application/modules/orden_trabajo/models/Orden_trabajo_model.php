...
class Orden_trabajo_model extends CI_Model
{
    ...
    public function getOrdenTrabajoTabla($where = '')
    {
        ...
        ot.TOTAL as TOTAL,
        IF(ot.DESCUENTO, ot.DESCUENTO, 0) AS DESCUENTO,
    ...
    public function getDetalleEstados($where = '')
    {
        $sql = "SELECT
                dot.ID_DETALLE_ORDEN_TRABAJO,
                dot.ID_ORDEN_TRABAJO,
                ent.ENTREGADO,
                pen.PENDIENTE
            FROM
            ...
            JOIN(
                SELECT
                    COUNT(ESTADO_SERVICIO) AS ENTREGADO
                FROM
                    detalle_orden_trabajo dot
                JOIN orden_trabajo ot ON ot.ID_ORDEN_TRABAJO=dot.ID_ORDEN_TRABAJO
                WHERE
                    ESTADO_SERVICIO = 4 $where
            ) AS ent
            JOIN(
                SELECT
                    COUNT(ESTADO_SERVICIO) AS PENDIENTE
                FROM
                    detalle_orden_trabajo dot
                JOIN orden_trabajo ot ON ot.ID_ORDEN_TRABAJO=dot.ID_ORDEN_TRABAJO
                WHERE
